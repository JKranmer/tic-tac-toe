...
const RULES: Record<GameMode, { 'pt-BR': string; en: string }> = {
  classic: {
    'pt-BR':
      'O modo Clássico segue as regras tradicionais do Jogo da Velha. O objetivo é alinhar três de seus símbolos (X ou O) na horizontal, vertical ou diagonal em um tabuleiro 3x3. O jogo termina quando um jogador consegue alinhar três símbolos ou quando o tabuleiro está cheio (empate).',
  ...
  infinite: {
    'pt-BR':
      'No modo Infinito, cada jogador só pode ter 3 peças no tabuleiro ao mesmo tempo. Ao tentar colocar a 4ª peça, a sua peça mais antiga será removida automaticamente. Isso torna o jogo dinâmico e evita empates, exigindo estratégia constante de movimento e reposicionamento.',
  ...
  ultimate: {
    'pt-BR':
      'O Ultimate Tic-Tac-Toe é jogado em um tabuleiro 3x3 grande, onde cada célula contém um mini-tabuleiro 3x3. Vencer um mini-tabuleiro marca aquela célula grande para você. Onde você joga em um mini-tabuleiro determina em qual mini-tabuleiro o oponente deve jogar na próxima vez. Vence quem alinhar 3 células grandes conquistadas.',
  ...
  gobble: {
    'pt-BR':
      "No modo Gobble, as peças têm tamanhos diferentes (Pequeno, Médio, Grande). Você pode colocar uma peça em uma casa vazia OU 'comer' (cobrir) uma peça menor (sua ou do oponente) com uma maior. No modo 'Classic' do Gobble, você também pode mover peças já no tabuleiro. O objetivo é alinhar 3 peças da sua cor visíveis.",
  ...
  bones: {
    'pt-BR':
      "No modo Saco de Ossos, o tabuleiro cresce dinamicamente. O objetivo é alinhar 3 Caveiras ou formar um 'Túmulo Perfeito' (quadrado 2x2 da sua cor). Caveiras aparecem quando um quadrado 2x2 de peças é formado. Você pode mover suas peças, desde que não quebre a conexão entre os ossos.",
    en: "In Bones mode, the board expands dynamically. The goal is to align 3 Skulls or form a 'Perfect Grave' (2x2 square of your color). Skulls appear when a 2x2 square of pieces is formed. You can move your pieces, provided you don't break the connection between the bones."
  }
};
...
const TITLES: Record<GameMode, { 'pt-BR': string; en: string }> = {
  ...
  bones: { 'pt-BR': 'Saco de Ossos', en: 'Bones (Bag of Bones)' }
};
        ...
        <>
          ...
          >
            ...
            <div className="bg-page p-4 rounded-xl border border-border/50 text-primary leading-relaxed">
              <p className="text-sm sm:text-base">{RULES[mode][lang]}</p>
            </div>
