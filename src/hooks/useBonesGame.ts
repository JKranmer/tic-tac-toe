...
export function useBonesGame(): BonesGameState {
  ...
  const isOccupied = useCallback(
    (x: number, y: number, currentPieces: Bone[] = pieces) => {
  ...
  const isConnectedWithout = useCallback((pieceToRemove: Bone, currentPieces: Bone[]) => {
    ...
    while (queue.length > 0) {
      ...
      const neighbors = remaining.filter(
        (p) =>
          !visited.has(p.id) &&
  ...
  const hasNeighbor = useCallback((x: number, y: number, currentPieces: Bone[]) => {
    return currentPieces.some((p) => (Math.abs(p.x - x) === 1 && p.y === y) || (Math.abs(p.y - y) === 1 && p.x === x));
  }, []);
  ...
  const checkWin = useCallback((currentSkulls: Skull[], currentPieces: Bone[]) => {
    ...
    for (const skull of currentSkulls) {
      // Check Horizontal
      if (
        skullMap.get(`${skull.x + 1},${skull.y}`)?.owner === skull.owner &&
        skullMap.get(`${skull.x + 2},${skull.y}`)?.owner === skull.owner
      ) {
        return {
          winner: skull.owner,
          line: [skull.id, `${skull.x + 1},${skull.y}`, `${skull.x + 2},${skull.y}`]
        };
      }
      // Check Vertical
      if (
        skullMap.get(`${skull.x},${skull.y + 1}`)?.owner === skull.owner &&
        skullMap.get(`${skull.x},${skull.y + 2}`)?.owner === skull.owner
      ) {
        return {
          winner: skull.owner,
          line: [skull.id, `${skull.x},${skull.y + 1}`, `${skull.x},${skull.y + 2}`]
        };
      }
      // Check Diagonal \
      if (
        skullMap.get(`${skull.x + 1},${skull.y + 1}`)?.owner === skull.owner &&
        skullMap.get(`${skull.x + 2},${skull.y + 2}`)?.owner === skull.owner
      ) {
        return {
          winner: skull.owner,
          line: [skull.id, `${skull.x + 1},${skull.y + 1}`, `${skull.x + 2},${skull.y + 2}`]
        };
      }
      // Check Diagonal /
      if (
  ...
  // Update Skulls Logic
  const updateSkulls = useCallback(
    (currentPieces: Bone[], currentPlayer: Player) => {
  ...
  const selectBone = (id: string | null) => {
    ...
    const neighbors = pieces.filter(
      (p) => (Math.abs(p.x - bone.x) === 1 && p.y === bone.y) || (Math.abs(p.y - bone.y) === 1 && p.x === bone.x)
    );
